...
public class ImprovedPlayerOld extends sheepdog.sim.Player {
	...
	public Point move(Point[] dogs, Point[] sheeps) {
		...
		if(endmod)
		{
			...
			dogs=newDogs;
			println("Endmodahead. this is="+(this.id-1));
			println(" no of dogs="+dogs.length);
		}
		...
		if(currState==State.INIT){
			...
			if(endmod==false && Math.max(
					furthestBlackSheep,
					mode==false?furthestWhiteSheep:Double.MIN_VALUE
					)<=distCutOff)
			{
				endmod=true;
				return dogs[this.id-1];
