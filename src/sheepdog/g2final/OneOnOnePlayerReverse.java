...
public class OneOnOnePlayerReverse extends sheepdog.sim.Player {
	...
	public int selectSheep(Point[] sheeps, Point dog){
		...
		ArrayList<Integer> whiteSheepOnRight=new ArrayList<Integer>();
		
		for(int i=nblacks;i<sheeps.length;i++)
		{
			if(sheeps[i].x<50)
				whiteSheepOnRight.add(i);
		}
		
		sheepFocus=whiteSheepOnRight.get(r.nextInt(whiteSheepOnRight.size()));
		
		/*
		r.nex
		sheepFocus++;
		if(sheepFocus==sheeps.length)
			sheepFocus=nblacks;
		while(sheepFocus>=nblacks && sheeps[sheepFocus].x>50)
		{
			sheepFocus++;
			if(sheepFocus==sheeps.length)
				sheepFocus=nblacks;
