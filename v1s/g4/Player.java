...
public class Player extends sheepdog.sim.Player {
    ...
    public Point move(Point[] dogs, // positions of dogs
    	...
    	//Calculates the new position of each sheep - where they will be 
    	//in the next tick
        ...
        if(move_num<26)
        {
            //Just move the dog to the gate for the first 26 steps
        ...
        if(dogs[id-1].x>=50){
        	...
        	for(int i=0;i<father.length;i++)
        	{
        		...
        		if(father[i]==-1)
        		{
        			println(String.format("sheep %d %f %f, father is -1",i,sheeps[i].x,sheeps[i].y));
        			continue;
        		}
        		else
        		{
        			
        			println(String.format("sheep %d %f %f, father is %d %f %f",
        					i,sheeps[i].x,sheeps[i].y,
        					father[i],sheeps[father[i]].x,sheeps[father[i]].y));
        		}
        	}
            ...
            //^^
            //This method will literally find the distance of the sheep from the root of the tree
            //to their current position, with the edges being path from those sheeps to their fathers
            //and their gates and store it in distanceInTree array
            ...
            println(String.format("Sheep chosen is %d, %f, %f",sheepid,sheeps[sheepid].x,sheeps[sheepid].y));
            
            //select farthest sheep and if it is close enough to it's pappa then select it's pappa
            if(sheepid!=-1 && father[sheepid]!=-1 && distance(sheeps[sheepid], sheeps[father[sheepid]])<1){
            	sheepid=father[sheepid];
            	System.out.println("worked!!!");
            	println(String.format("NEWWWWWWWWWWWWWWWWWWWWWW\n sheep chosen is %d, %f, %f",sheepid,sheeps[sheepid].x,sheeps[sheepid].y));
            }
            ...
            if(father[sheepid]==-1){
            	toPoint=gate;
            }else toPoint= sheeps[father[sheepid]]; //from sheep to it's father
            
            //position this one such that it goes to its father
	...
	private int[] findFarthest(int k) {
    	...
    	for (int j = 0; j < d.length; j++) {
			d[j]=distanceInTree[j]-distance(dogs[id-1], sheeps[j])*1.5; //no idea what this 1.5 is
		}
    	for (int j = 0; j < d.length; j++) {
			if (sheeps[j].x<50)
				d[j]=-100;
    	...
    	for (int i = 0; i < index.length; i++) {
			index[i]=i; // runs for each sheep
		}
    	
    	
		for (int i = 0; i < distanceInTree.length; i++) {
			for (int j = i+1; j < distanceInTree.length; j++) {
				//for each pair of sheeps, if current dog is farther from j than i
				if (d[i]<d[j]) {
					double t=d[i];
					d[i]=d[j];
					d[j]=t;
		...
		return ret; //arrange dogs in some order - increasing or decreasin effective distance from gate
	}
