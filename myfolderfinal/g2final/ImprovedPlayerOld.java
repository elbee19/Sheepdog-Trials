...
public class ImprovedPlayerOld extends sheepdog.sim.Player {
	...
	//MOD VARIABLE
	private int dogCutOff=6;
	private int dogCutOff2=20;
	private double distCutOff=12;
	...
	private boolean takeCareOfYellowSheep=false;
	...
	public ImprovedPlayerOld()
	{
		...
		furthestWhiteSheep=Double.MIN_VALUE;
	...
	public Point move(Point[] dogs, Point[] sheeps) {
		...
		if(this.id>dogCutOff2)
			return dogs[this.id-1];
		
		if(dogs.length>dogCutOff2)
		{
			Point[] newDogs2=new Point[dogCutOff2];
			
			System.arraycopy(dogs, 0, newDogs2, 0, dogCutOff2);
			dogs=newDogs2;
		}
		if(endmod && dogs.length>dogCutOff)
		{
			println("Endmod");
			if(this.id>dogCutOff)
			{
				dogs[this.id-1].x+=1.99;
				return dogs[this.id-1];
			}
			
			Point[] newDogs=new Point[dogCutOff];
			System.arraycopy(dogs, 0, newDogs, 0, dogCutOff);
			dogs=newDogs;
		...
		if(currState==State.INIT){
			...
			println(String.format("max dist is %f, maxblck %f maxwhit %f"
					,Math.max(
							furthestBlackSheep,
							mode==false?furthestWhiteSheep:furthestBlackSheep
							),furthestBlackSheep,furthestWhiteSheep));
			if(endmod==false && Math.max(
					furthestBlackSheep,
					mode==false?furthestWhiteSheep:furthestBlackSheep
					)<=distCutOff)
			{
	...
	public boolean[] mapSheep2(Point[] dogs, Point[] sheep, int dogSectorId) {
		...
		{	
			...
			else //white
			{
				...
				if(furthestWhiteSheep==Double.MIN_VALUE || gateDistance>=distance(gatePoint,sheep[furthestBlackPoint]))
				{
					furthestWhiteSheep=gateDistance;
					furthestWhitePoint=i;
				}
			}
